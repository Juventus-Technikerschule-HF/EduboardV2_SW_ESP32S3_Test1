...
******************************************************************************/
#define SPI_FREQUENCY_SLOW_300KHZ (80 * 1000 * 1000 / 10)
spi_device_handle_t sd_spi_slow_device;
spi_device_handle_t sd_spi_fast_device;
SDRESULTS SD_Init(SD_DEV *dev)
{
    ...
    for(init_trys=0; ((init_trys!=SD_INIT_TRYS)&&(!ct)); init_trys++)
    {
        ...
        // -- SPI_Init();
        gpspi_init(&sd_spi_slow_device, GPIO_MOSI, GPIO_MISO, GPIO_SCK, GPIO_SD_CS, SPI_FREQUENCY_SLOW_300KHZ, false);
        gpspi_init(&sd_spi_fast_device, GPIO_MOSI, GPIO_MISO, GPIO_SCK, GPIO_SD_CS, SPI_FREQUENCY_SLOW_300KHZ, false);
