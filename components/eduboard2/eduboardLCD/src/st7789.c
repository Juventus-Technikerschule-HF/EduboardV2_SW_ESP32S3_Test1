...
void spi_master_init(TFT_t * dev, int16_t pin_mosi, int16_t pin_sck, int16_t pin_cs, int16_t pin_dc, int16_t pin_reset, int16_t pin_bl)
{
	...
	ESP_LOGI(TAG, "GPIO_DC=%d",pin_dc);
	//gpio_pad_select_gpio( GPIO_DC );
	gpio_reset_pin( pin_dc );
	gpio_set_direction( pin_dc, GPIO_MODE_OUTPUT );
	gpio_set_level( pin_dc, 0 );

	ESP_LOGI(TAG, "GPIO_RESET=%d",pin_reset);
	if ( pin_reset >= 0 ) {
	...
	ESP_LOGI(TAG, "GPIO_MOSI=%d",pin_mosi);
	ESP_LOGI(TAG, "GPIO_SCLK=%d",pin_sck);
...
void lcdUpdateVScreen(TFT_t * dev) {		
	for(int y = 0; y < dev->_height; y++) {
		...
		uint16_t _y2 = _y1;
		uint16_t * colors = &vScreen.data[y*dev->_width];
		spi_master_write_command(dev, 0x2A);	// set column(x) address
		spi_master_write_addr(dev, _x1, _x2);
		spi_master_write_command(dev, 0x2B);	// set Page(y) address
		spi_master_write_addr(dev, _y1, _y2);
		spi_master_write_command(dev, 0x2C);	//	Memory Write
		spi_master_write_colors(dev, colors, dev->_width);
	}
}
...
void lcdDrawPixel(TFT_t * dev, uint16_t x, uint16_t y, uint16_t color){
	...
	if(vScreen.enabled == false) {
		...
		vScreen.data[(_y * dev->_width) + _x] = color;
	}
}
...
void lcdDrawMultiPixels(TFT_t * dev, uint16_t x, uint16_t y, uint16_t size, uint16_t * colors) {
	...
	if(vScreen.enabled == false) {
		...
		for(int i = 0; i < size; i++) {
			vScreen.data[(_y1 * dev->_width) + _x1 + i] = colors[i];
		}
	}
}
